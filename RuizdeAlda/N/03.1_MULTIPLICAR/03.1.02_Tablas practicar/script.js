document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Funciones de Población de Selects (MODIFICADA: Tabla) ---
    ...
    // --- Lógica de Botones de Resultado (Sin cambios) ---
    ...
    // --- Lógica del Juego (MODIFICADA: initializeCards para el título) ---
    
    function generateOperation() {
        const table = selectedTable || 1; 
    ...
    function initializeCards(resetAll = false) {
        // *** CONDICIÓN DE INICIO MODIFICADA ***
        if (!selectedTable) {
            cardContainer.innerHTML = '<p style="font-weight: bold; color: #D61C00;">Selecciona una tabla para empezar.</p>';
            if (isGameRunning) pauseGame();
            startButton.disabled = true;
            document.querySelector('header h1').textContent = `Tablas de Multiplicar`; // Reiniciar título
            return;
        }
        // **********************************
    ...
    function startGame() {
        if (isGameRunning || !selectedTable) return;
